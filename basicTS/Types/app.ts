...
// 타입스크립트는 컴파일까지 막진 않는다. 그러니까 타입스크립트는 런타임중에 작동하지 않는다.

// 자바스크립트는 동적 타이핑
// 타입스크립트는 정적 타이핑

// 동적 타이핑은 할당시점에(런타임시) 컴퓨터가 타입을 정하는 것이고, 정적 타이핑은 할당 전에 미리 개발자가 미리 타입을 선언하는 것을 말한다.

// 타입스크립트가 타입을 자동으로 추론하게 두는 것이 좋다.
...
// 화살표 함수와 타입
const func = (output) => console.log(output);

// rest 문법과 타입 + 튜플
const addNums = (...numbers: number[]) => {};

const addNumber = addNums(1, 2, 5, 7);
